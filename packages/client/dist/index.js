...
async function getClient(cfg) {
    ...
    const requestInit = cfg.headers ? { headers: cfg.headers } : {};
...
async function callTalonpress(cfg, toolName, args) {
    try {
 ...
 * `process.cwd()`.
 */
export function getTalonpressTools(cfg, workspaceDir) {
    const resolveFolder = (folder) => path.isAbsolute(folder) ? folder : path.join(workspaceDir ?? process.cwd(), folder);
    return {
        talonpress_publish: tool({
            description: 'Publish or update a static web package on TalonPress by uploading selected files from a local folder. ' +
            ...
            inputSchema: z.object({
                ...
                files: z
                    .array(z.string())
                    .optional()
                    .describe('Relative paths (to `folder`) to publish. Each entry may be a file or a subdirectory ' +
                ...
                default_page: z
                    .string()
                    .describe('Entry-point file served at the package root (e.g. index.html). ' +
                ...
                context: z
                    ...
                    .optional()
                    .describe('What an agent should know about this package when a reader chats about it from the ' +
            ...
            execute: async (input) => {
                try {
                    ...
                    if (!stat.isDirectory())
                        return `Error: not a directory: ${input.folder}`;
        ...
        talonpress_list_packages: tool({
            ...
            execute: async (input) => {
                try {
                    return await callTalonpress(cfg, 'list_packages', input);
                }
                catch (err) {
                    return `Error: ${err instanceof Error ? err.message : String(err)}`;
                }
        ...
        talonpress_get_package_status: tool({
            description: 'Fetch the live status, route configuration, file manifest, and active tokens for a TalonPress package.',
            inputSchema: z.object({
                package_id: z.string().describe('Package ID.'),
            }),
            execute: async (input) => {
                try {
                    return await callTalonpress(cfg, 'get_package_status', input);
                }
                catch (err) {
                    return `Error: ${err instanceof Error ? err.message : String(err)}`;
                }
        ...
        talonpress_update_visibility: tool({
            description: 'Modify the access visibility of a TalonPress package. ' +
            ...
            execute: async (input) => {
                try {
                    return await callTalonpress(cfg, 'update_visibility', input);
                }
                catch (err) {
                    return `Error: ${err instanceof Error ? err.message : String(err)}`;
                }
        ...
        talonpress_renew_token: tool({
            description: 'Rotate the secure_token of a private TalonPress package. ' +
            ...
            execute: async (input) => {
                try {
                    return await callTalonpress(cfg, 'renew_token', input);
                }
                catch (err) {
                    return `Error: ${err instanceof Error ? err.message : String(err)}`;
                }
        ...
        talonpress_delete_package: tool({
            ...
            execute: async (input) => {
                try {
                    return await callTalonpress(cfg, 'delete_package', input);
                }
                catch (err) {
                    return `Error: ${err instanceof Error ? err.message : String(err)}`;
                }
